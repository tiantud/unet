...
class UNet_multiout(nn.Module):

    def __init__(self, in_channels=3, out_channels=1, init_features=32):
        super(UNet_multiout, self).__init__()

        features = init_features
        self.encoder1 = UNet_multiout._block(in_channels, features, name="enc1")
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder2 = UNet_multiout._block(features, features * 2, name="enc2")
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder3 = UNet_multiout._block(features * 2, features * 4, name="enc3")
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder4 = UNet_multiout._block(features * 4, features * 8, name="enc4")
        self.pool4 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.bottleneck = UNet_multiout._block(features * 8, features * 16, name="bottleneck")

        self.upconv4 = nn.ConvTranspose2d(
            features * 16, features * 8, kernel_size=2, stride=2
        )
        self.outconv4 = nn.ConvTranspose2d(
            features * 16, out_channels, kernel_size=2, stride=16
        )
        self.decoder4 = UNet_multiout._block((features * 8) * 2, features * 8, name="dec4")
        self.upconv3 = nn.ConvTranspose2d(
            features * 8, features * 4, kernel_size=2, stride=2
        )
        self.outconv3 = nn.ConvTranspose2d(
            features * 8, out_channels, kernel_size=2, stride=8
        )
        self.decoder3 = UNet_multiout._block((features * 4) * 2, features * 4, name="dec3")
        self.upconv2 = nn.ConvTranspose2d(
            features * 4, features * 2, kernel_size=2, stride=2
        )
        self.outconv2 = nn.ConvTranspose2d(
            features * 4, out_channels, kernel_size=2, stride=4
        )
        self.decoder2 = UNet_multiout._block((features * 2) * 2, features * 2, name="dec2")
        self.upconv1 = nn.ConvTranspose2d(
            features * 2, features, kernel_size=2, stride=2
        )
        self.outconv1 = nn.ConvTranspose2d(
            features * 2, out_channels, kernel_size=2, stride=2
        )
        self.decoder1 = UNet_multiout._block(features * 2, features, name="dec1")
    ...
    def forward(self, x):
        ...
        out_size = dec1.size()[2:]

        out_list = [
            torch.sigmoid(self.outconv4(bottleneck, output_size=out_size)),
            torch.sigmoid(self.outconv3(dec4, output_size=out_size)),
            torch.sigmoid(self.outconv2(dec3, output_size=out_size)),
            torch.sigmoid(self.outconv1(dec2, output_size=out_size)),
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, mask, fname in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss_list = list( criterion(outputs, mask) for outputs in outputs_list )
...
def do_segmentation(model, *, dataset_sizes, dataloaders, title, maskRGB, predRGB):
    ...
    for inputs, mask, fname in dataloaders['val']:
        ...
        outputs = (model(inputs)[-1] > threshold).float() 
...
for crossval_element in CROSSVAL_LIST:
    ...
    model = UNet_multiout(in_channels=3, out_channels=1)
    model.to(device)
    ...
    for padding in ['_LL', '_LE']:
        fresh_model = UNet_multiout(in_channels=3, out_channels=1)
