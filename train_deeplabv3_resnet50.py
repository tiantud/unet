...
# start date
DESCRIPTION = 'deeplabv3_resnet50'
...
def do_segmentation(model, *, dataset_sizes, dataloaders, title, maskRGB, predRGB):
    ...
    for inputs, inputs_no_normalize, mask, fname in dataloaders['val']:
        ...
        outputs_edged = (torch.nn.functional.conv2d(outputs_edged, kernel, padding=1) > 0).float()
        mask_edged = (torch.nn.functional.conv2d(mask_edged, kernel, padding=1) > 0).float()
...
for crossval_element in CROSSVAL_LIST:
    ...
    for padding in ['_LL', '_LE']:
        fresh_model = torchvision.models.segmentation.deeplabv3_resnet50(num_classes=2)
