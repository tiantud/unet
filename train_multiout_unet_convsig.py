...
class ConfusionMatrix(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        y_pred = y_pred.view(-1).to(torch.int32)
        y_true = y_true.view(-1).to(torch.int32)
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=1, init_features=32):
        ...
        self.outconv4 = nn.ConvTranspose2d(
            features * 16, int(init_features / 16), kernel_size=2, stride=16
        )
        ...
        self.outconv3 = nn.ConvTranspose2d(
            features * 8, int(init_features / 8), kernel_size=2, stride=8
        )
        ...
        self.outconv2 = nn.ConvTranspose2d(
            features * 4, int(init_features / 4), kernel_size=2, stride=4
        )
        ...
        self.outconv1 = nn.ConvTranspose2d(
            features * 2, int(init_features / 2), kernel_size=2, stride=2
        )
        self.decoder1 = UNet._block(features * 2, features, name="dec1")

        self.conv = nn.Conv2d(
            in_channels=features * 2 - 2, out_channels=out_channels, kernel_size=1
        )
    ...
    def forward(self, x):
        ...
        out4 = self.outconv4(bottleneck, output_size=out_size)
        out3 = self.outconv3(dec4, output_size=out_size)
        out2 = self.outconv2(dec3, output_size=out_size)
        out1 = self.outconv1(dec2, output_size=out_size)
        out0 = dec1

        out = torch.cat((out4, out3, out2, out1, out0), dim=1)
...
def train_model(model, *, dataset_sizes, dataloaders, title, criterion, optimizer, scheduler, num_epochs=25):
    ...
    print('-' * 50)
    print('Training started at: {}'.format(datetime.today().strftime('%Y%m%d_%H%M%S')))
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            print('{} Loss: {} percision: {} recall: {} accuracy: {}'.format(
                phase, 
                epoch_loss, 
...
def do_segmentation(model, *, dataset_sizes, dataloaders, title):
    ...
    for inputs, mask, fname in dataloaders['val']:
        ...
        torchvision.utils.save_image(inputs, os.path.join(img_folder_path, fname[0]))
        torchvision.utils.save_image(mask, os.path.join(img_folder_path, mname))
        torchvision.utils.save_image(outputs, os.path.join(img_folder_path, oname))
...
training_inf = {
    'model': 'train_multiout_unet_convsig',
    'DATE_INFO': DATE_INFO,
    'LR': LR,
